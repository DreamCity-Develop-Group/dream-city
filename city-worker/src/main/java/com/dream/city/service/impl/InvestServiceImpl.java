    ...
    public void profitGrant(CityInvest invest) {
        ...
        for (InvestRule rule : rules ) {
            ...
            if(Constants.ALL_ORDERS.equalsIgnoreCase(ruleFlag)){//所有订单收益
                ...
                for (int i=0;i<orders.size();i++) {
                    ThreadPoolUtil.submit(ThreadPoolUtil.poolCount, ThreadPoolUtil.MODULE_MESSAGE_RESEND,
                            new OrderProfitThead(firstTimeOrdersProfitPer,orders.get(i),this,playerAccountMapper,accountService,playerEarningService,endGate));
                    if(i==orders.size()-1){
                        endGate.countDown();
                    }
                }
                ...
                for (int i=0;i<orders.size();i++) {
                    ThreadPoolUtil.submit(ThreadPoolUtil.poolCount, ThreadPoolUtil.MODULE_MESSAGE_RESEND,
                            new OrderProfitThead(everyTopMmberProfit,orders.get(i),this,playerAccountMapper,accountService,playerEarningService,topMemberendGate));
                    if(i==orders.size()-1){
